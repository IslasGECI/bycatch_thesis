# ==========================================
# Título: Grafica los hot spots de congestión VMS como mapa binario
#
# Contexto (Por qué):
...
# celda para los puntos VMS. Filtra las celdas con al menos
# un punto VMS para evitar saturar el mapa con celdas vacías.
# Clasifica cada celda como hot spot (z-score >= 1.96) o no
# hot spot (z-score < 1.96). Construye un mapa con ggplot2
# usando naranja para hot spots y verde para no hot spots,
# y superpone la línea de costa mundial como contexto
# geográfico.
#
# Entradas:
...
#
# Notas:
# - Solo grafica celdas con al menos un punto VMS
...
# los puntos VMS en la rejilla del KDE
...
# spots de congestión VMS en el Pacífico mexicano
output_png_path <- "reports/figures/vms_hotspot_binary_map.png"
...
# (z-score >= 1.96) donde la congestión de embarcaciones se
# agrupa significativamente en el espacio
hot_spot_fill_color <- "#E67E22"

# Color verde para las celdas clasificadas como no hot spot
# (z-score < 1.96) donde la congestión de embarcaciones no
# muestra agrupación espacial significativa
...
# con las celdas vacías que no aportan información sobre
...
# zonas de agrupación significativa de congestión VMS
hotspot_map <- ggplot() +
  ...
  # Capa de las celdas con puntos VMS coloreadas según la
  # clasificación binaria: naranja para hot spots con
  # agrupación significativa y verde para no hot spots
  # donde la congestión no muestra patrón espacial
  geom_sf(
    data = grid_nonzero,
  ...
  # spots verdaderos y verde a los no hot spots, con
  # etiquetas descriptivas en español para la interpretación
  # del mapa de congestión de embarcaciones
  scale_fill_manual(
    values = c(
  ...
  # Limita la extensión del mapa al bounding box de zoom in
  # para enfocar la visualización en la región de la ZEE de
  # México donde se concentran los puntos VMS con congestión
  # significativa de embarcaciones pesqueras
  coord_sf(
  ...
  # hot spots para los datos de congestión VMS de
  # embarcaciones pesqueras en el Pacífico mexicano
  labs(
    title = "Hot spots de congestión VMS",
