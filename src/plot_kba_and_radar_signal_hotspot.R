# ==========================================
# Título: Grafica el mapa combinado del sitio potencial KBA con los hot spots de señal de radar
#
# Contexto (Por qué):
# El polígono del KBA (potentialSite = TRUE) delimita la zona de
# alimentación relevante del albatros de Laysan. Las celdas hot
# spot de señal de radar indican zonas con agrupación significativa
# de alta intensidad de señal de radar del albatros. El mapa
# combinado superpone ambas capas para visualizar la alta intensidad
# de señal dentro del área de alimentación del albatros.
#
# Descripción (Qué / Cómo):
# Lee los polígonos KBA, las celdas hot spot de señal de radar, la
# intersección KBA ∩ hot spot de señal de radar, las AMP, la ZEE y
...
#   las celdas con alta intensidad significativa dentro del KBA
...
# de señal de radar en la rejilla del KDE
input_hotspot_path <- "data/processed/radar_signal_hotspot.gpkg"

# Ruta del GeoPackage con la intersección KBA ∩ hot spot de señal de
# radar para resaltar las celdas con alta intensidad significativa
# dentro del KBA
input_intersection_path <- "data/processed/kba_radar_signal_hotspot_intersection.gpkg"
...
# Ruta del archivo PNG que almacenará el mapa combinado de KBA y hot
# spots de señal de radar en el Pacífico mexicano
output_figure_path <- "reports/figures/kba_and_radar_signal_hotspot_map.png"
...
# generado por src/compute_radar_signal_hotspot.R
radar_signal_hotspot_sf <- st_read(input_hotspot_path, quiet = TRUE)

# Importa la intersección KBA ∩ hot spot de señal de radar desde el
# GeoPackage generado por src/export_kba_radar_signal_hotspot_intersection.R
kba_radar_signal_hotspot_intersection_sf <- st_read(input_intersection_path, quiet = TRUE)
...
# que renderiza mapSite(), y las fusiona en un solo contorno para graficarlo
# como relleno de referencia del área de alimentación
kba_union_sf <- kba_polygons_sf |>
  filter(potentialSite == TRUE) |>
  summarise()

# Filtra la rejilla para conservar solo las celdas con suma de señal de
# radar positiva para evitar saturar la visualización con las celdas vacías
# que no aportan información sobre intensidad de señal de radar
grid_nonzero_sf <- radar_signal_hotspot_sf |>
...
# señal de radar en la región de la ZEE de México
bbox_lon_min <- -125
bbox_lon_max <- -105
bbox_lat_min <- 15
bbox_lat_max <- 35
...
# relación entre el sitio potencial KBA y los hot spots de señal de radar
plot_kba_hotspot_combined <- ggplot() +
  ...
  # Capa de las celdas con suma de señal de radar positiva coloreadas
  # según la clasificación binaria: naranja para hot spots con agrupación
  ...
  # las celdas de señal de radar para que el color púrpura oculte las
  # celdas verdes dentro del área de alimentación del albatros. El mapeo
  # a fill con una constante genera una entrada en la leyenda para el KBA
  geom_sf(
    data = kba_union_sf,
  ...
  # Capa de la intersección KBA ∩ hot spot de señal de radar con relleno
  # rojo que se dibuja sobre el KBA para resaltar las celdas con alta
  # intensidad significativa dentro del área de alimentación del albatros
  geom_sf(
    data = kba_radar_signal_hotspot_intersection_sf,
  ...
  labs(
    title = "Potential KBA with Radar Signal Hotspots",
