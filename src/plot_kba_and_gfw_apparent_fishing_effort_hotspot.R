# ==========================================
# Título: Grafica el mapa combinado del sitio potencial KBA con los hot spots de esfuerzo pesquero de GFW
#
# Contexto (Por qué):
# El polígono del KBA (potentialSite = TRUE) delimita la zona de
# alimentación relevante del albatros de Laysan. Las celdas hot
# spot de esfuerzo pesquero de GFW indican zonas con agrupación
# significativa de presión pesquera. El mapa combinado superpone
# ambas capas para visualizar la presión pesquera dentro del
# área de alimentación del albatros.
#
# Descripción (Qué / Cómo):
# Lee los polígonos KBA, las celdas hot spot de esfuerzo pesquero de
# GFW, la intersección KBA ∩ hot spot, las AMP, la ZEE y la costa.
# Filtra el KBA a potentialSite = TRUE. Construye un mapa con ggplot2
# que superpone las celdas hot spot, el relleno púrpura del KBA, el
# relleno rojo de la intersección, la ZEE, las AMP y la costa. Usa el
# bounding box hardcodeado de la intersección KBA como extensión.
#
# Entradas:
...
# Ruta del GeoPackage con la intersección KBA ∩ hot spot de esfuerzo
# pesquero de GFW para resaltar las celdas con presión pesquera
# significativa dentro del KBA
input_intersection_path <- "data/processed/kba_gfw_apparent_fishing_effort_hotspot_intersection.gpkg"
...
# spots de esfuerzo pesquero de GFW en el Pacífico mexicano
output_figure_path <- "reports/figures/kba_and_gfw_apparent_fishing_effort_hotspot_map.png"
...
gfw_apparent_fishing_effort_hotspot_sf <- st_read(input_hotspot_path, quiet = TRUE)

# Importa la intersección KBA ∩ hot spot desde el GeoPackage generado por
# src/export_kba_fishing_effort_hotspot_intersection.R
kba_gfw_apparent_fishing_effort_hotspot_intersection_sf <- st_read(input_intersection_path, quiet = TRUE)
...
# pesca aparente positiva para evitar saturar la visualización con las
# celdas vacías que no aportan información sobre presión pesquera
grid_nonzero_sf <- gfw_apparent_fishing_effort_hotspot_sf |>
...
# esfuerzo pesquero de GFW en la región de la ZEE de México
bbox_lon_min <- -125
bbox_lon_max <- -105
bbox_lat_min <- 15
bbox_lat_max <- 35
...
# relación entre el sitio potencial KBA y los hot spots de esfuerzo
# pesquero de GFW
plot_kba_hotspot_combined <- ggplot() +
  ...
  # Capa de las celdas con horas de pesca aparente positivas coloreadas
  ...
  # las celdas de esfuerzo pesquero para que el color púrpura oculte las
  ...
  geom_sf(
    data = kba_gfw_apparent_fishing_effort_hotspot_intersection_sf,
  ...
  labs(
    title = "Potential KBA with GFW Apparent Fishing Effort Hotspots",
