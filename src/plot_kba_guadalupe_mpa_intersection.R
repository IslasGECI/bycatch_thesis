# ==========================================
# Título: Grafica la intersección entre el sitio potencial KBA de Guadalupe y las AMP de México
#
# Contexto (Por qué):
# El polígono del KBA (potentialSite = TRUE) delimita la zona de
# alimentación relevante del albatros de Laysan en la Isla Guadalupe.
# Las Áreas Marinas Protegidas (AMP) definen zonas marinas bajo protección
# legal. Su intersección revela qué fracción del sitio potencial KBA de
# Guadalupe está protegida dentro del sistema de AMP.
#
# Descripción (Qué / Cómo):
# Lee el GeoPackage de la intersección KBA ∩ AMP (calculada solo con celdas
# potentialSite = TRUE), los polígonos KBA de Guadalupe, las AMP, la ZEE y
# la costa. Filtra el KBA a potentialSite = TRUE y lo fusiona en un contorno
# relleno. Construye un mapa con ggplot2 que superpone la intersección
# destacada sobre el relleno púrpura del KBA, las AMP, la ZEE y la costa.
# Usa el bounding box de zoom in como extensión.
#
# Entradas:
...
# - La intersección ya viene pre-calculada por export_kba_guadalupe_mpa_intersection.R
# ==========================================
...
# export_kba_guadalupe_mpa_intersection.R (ya filtrada a potentialSite = TRUE)
kba_mpa_intersection_sf <- st_read(input_intersection_path, quiet = TRUE)
# Importa los polígonos KBA originales para extraer el relleno púrpura del
...
# que renderiza bycatch::render_potential_kba(), y las fusiona en un solo
# contorno para graficarlo como referencia
...
# California para mostrar la intersección del KBA de Guadalupe con las AMP
bbox_lon_min <- -125
bbox_lon_max <- -105
bbox_lat_min <- 15
bbox_lat_max <- 35
