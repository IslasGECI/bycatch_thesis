# ==========================================
# Título: Grafica los hot spots de palangre de GFW
#
# Contexto (Por qué):
# Los z-scores de Getis-Ord Gi* miden la intensidad de
# presión pesquera por palangre en cada celda de la rejilla
# del KDE. Visualizar la gradación espacial del estadístico
# permite interpretar dónde se concentran los eventos de
# pesca con palangre en el Pacífico mexicano.
#
...
# mapa con celdas vacías. Aplica log1p (log(1+x)) al z-score
# para comprimir la cola superior y revelar la gradación en
# los valores cercanos a cero. Construye un mapa con ggplot2
# usando la paleta inferno y superpone la línea de costa
# mundial como contexto geográfico.
#
# Entradas:
...
output_png_path <- "reports/figures/gfw_longline_hotspot_map_all.png"
...
# 'inferno' ofrece una gradación oscuro a brillante perceptualmente
...
grid_nonzero <- longline_hotspot_sf |>
  ...
  # cola superior (valores extremos) y expandir los valores
  # cercanos a cero donde se concentra la mayor parte de las
  # celdas; log1p maneja ceros sin problemas
  mutate(
...
hotspot_map <- ggplot() +
  ...
  # Capa de las celdas con puntos de palangre coloreadas por
  # el logaritmo del z-score para revelar la gradación en la
  ...
  # Etiquetas del mapa descriptivas del análisis THS para
  # los eventos de palangre de Global Fishing Watch
  labs(
    title = "Hot spots de eventos de palangre de GFW",
    subtitle = "Getis-Ord Gi* sobre datos GFW en el Pacífico Mexicano",
    x = "Longitud",
    y = "Latitud"
  ) +
...
# su inclusión en reportes y presentaciones del proyecto de
