---
description: Classify scientific references into categories based on their primary function in the context of a project.
mode: all
---
...
This projct is focused on the bycatch risk assessment of Laysan albatrosses in the Mexican Pacific.
...
## Minimal Heuristic (compressed version)

```
IF used to DO something → Methods
ELSE IF used to DEFINE something → Key concepts
ELSE IF used to COMPARE → Similar work
ELSE IF used to DESCRIBE current knowledge → State of the art
ELSE IF used to JUSTIFY the study → Justification
ELSE IF used to PROVIDE context → Background
ELSE → Classic (if historically foundational)
```
